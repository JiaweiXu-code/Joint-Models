libname lib "/pine/scr/j/i/jiawei/rev11/mis/random0.5/means_re";

data all&sysparm.;
infile "/pine/scr/j/i/jiawei/rev11/mis/random0.5/data/data&sysparm..csv" delimiter = ',';
...
run;
	...
	** set parameter name based on different source datasets;
	if Effect > '' then parameter = Effect;
        ...
        else if c and upcase(CovParm) = 'UN(2,2)' then parameter = 'linear';
	...
	if c then do;
	 ...
	 if find(variable,'intercept','i')   then do;  variable = 'random_intercept'; expression = 'theta0';         end;
         else if find(variable,'linear','i') then do;  variable = 'random_linear';    expression = 'theta1';    end;     
...
bounds sig_intercept>1e-3, sig_time>1e-3, sig_residual>1e-3; 
v11 = sig_intercept**2;
v22 = sig_time**2;
random random_intercept random_linear ~ normal([0,0],[v11,0,v22]) subject=id;

  ** longitudinal component;
    muLong = random_intercept + random_linear*time + gamma_intercept + gamma_t1*t1 + gamma_t2*t2 + gamma_t3*t3 + gamma_t4*t4 + gamma_trt*trt + (gamma_t1_trt*t1+gamma_t2_trt*t2+gamma_t3_trt*t3+gamma_t4_trt*t4)*trt + gamma_nodegrp*nodegrp;
   ...
   if surv_like = 1 then do;
	...
	A2 = A1 + beta*(gamma_t1 + gamma_t1_trt*trt + random_linear)*L1;
	A3 = A2 + beta*(gamma_t2 + gamma_t2_trt*trt + random_linear)*(L2-L1);
        A4 = A3 + beta*(gamma_t3 + gamma_t3_trt*trt + random_linear)*(L3-L2);

        B1 = beta*(gamma_t1 + gamma_t1_trt*trt + random_linear);
        B2 = beta*(gamma_t2 + gamma_t2_trt*trt + random_linear);
  	B3 = beta*(gamma_t3 + gamma_t3_trt*trt + random_linear);
        B4 = beta*(gamma_t4 + gamma_t4_trt*trt + random_linear);
