...
##################### general case ######################
JMD = function(seed, v, k = 2.85, nlinear = 4, measures = 9, npieces = 5, eta = 1, maxt = 10, censor = 5, 
               censorp = 0.05, interval = 0.001, pknots = c(0.25,0.75,1.25), knots = c(1.91, 2.43, 3.00, 3.80), 
               sigma = 0.65642581, p = c(0.5,0.5), mSigma = 0.71202084, mk = 0.5, gamma = c(0.26634693,0,-0.03393003),
  ...
  for (i in 1:ss){
    ...
    ### trajectory function ###
    mu = theta + theta1*tpoints + plinear %*% t(g) + pinter %*% t(g*z) + as.numeric(gamma %*% rbind(1,z,node))
    ### longitudinal data ###
    ...
    hazards = function(t){
      ...
      mu = theta + theta1*t + plinear %*% gg + pinter %*% gg*z + gamma[1:2]%*%rbind(1,z)
      if (npieces == 1){
...
for (ii in ((temp-1)*com+1):(com*temp)){
  ...
  means = c(node.idx,mean_dur,sample,event)
...
fname1 = paste("/pine/scr/j/i/jiawei/rev11/mis/random0.5/data/data",node.idx,".csv",sep = "")
