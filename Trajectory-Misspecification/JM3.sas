libname lib "/pine/scr/j/i/jiawei/rev11/mis/leveloff/means";
...
by sim;
 model measure = t1 t2 t3 trt t1*trt t2*trt t3*trt nodegrp / solution;
...
random random_intercept ~ normal(0,v11) subject=id;
  ...
  ** longitudinal component;
    muLong = random_intercept + gamma_intercept + gamma_t1*t1 + gamma_t2*t2 + gamma_t3*t3 + gamma_trt*trt + (gamma_t1_trt*t1+gamma_t2_trt*t2+gamma_t3_trt*t3)*trt + gamma_nodegrp*nodegrp;
	...
	** when beta_joint approximation zero the hazard is constant as a function of time and hence the likelihood contribution is different;	  
	...
	if t <= k1 then do;
	    ...
	    else logLike = logLike + censorship*(alpha_s1+A3+B3*(t-L2)) - exp(alpha_s1+A1)*(exp(B1*L1)-1)/B1 - exp(alpha_s1+A2)*(exp(B2*(L2-L1))-1)/B2 - exp(alpha_s1+A3)*(exp(B3*(t-L2))-1)/B3;
        end;
          
	else if t <= k2 then logLike = logLike + censorship*(alpha_s2+A3+B3*(t-L2)) - exp(alpha_s2+A3)/B3*(exp(B3*(t-L2))-exp(B3*(k1-L2))) - exp(alpha_s1+A1)*(exp(B1*L1)-1)/B1 - exp(alpha_s1+A2)*(exp(B2*(L2-L1))-1)/B2 - exp(alpha_s1+A3)*(exp(B3*(k1-L2))-1)/B3;
        else if t <= k3 then logLike = logLike + censorship*(alpha_s3+A3+B3*(t-L2)) - exp(alpha_s3+A3)/B3*(exp(B3*(t-L2))-exp(B3*(k2-L2))) - exp(alpha_s2+A3)/B3*(exp(B3*(k2-L2))-exp(B3*(k1-L2))) - exp(alpha_s1+A1)*(exp(B1*L1)-1)/B1 - exp(alpha_s1+A2)*(exp(B2*(L2-L1))-1)/B2 - exp(alpha_s1+A3)*(exp(B3*(k1-L2))-1)/B3;
	else if t <= k4 then logLike = logLike + censorship*(alpha_s4+A3+B3*(t-L2)) - exp(alpha_s4+A3)/B3*(exp(B3*(t-L2))-exp(B3*(k3-L2))) - exp(alpha_s3+A3)/B3*(exp(B3*(k3-L2))-exp(B3*(k2-L2))) - exp(alpha_s2+A3)/B3*(exp(B3*(k2-L2))-exp(B3*(k1-L2))) - exp(alpha_s1+A1)*(exp(B1*L1)-1)/B1 - exp(alpha_s1+A2)*(exp(B2*(L2-L1))-1)/B2 - exp(alpha_s1+A3)*(exp(B3*(k1-L2))-1)/B3;
        else logLike = logLike + censorship*(alpha_s5+A3+B3*(t-L2)) - exp(alpha_s5+A3)/B3*(exp(B3*(t-L2))-exp(B3*(k4-L2))) - exp(alpha_s4+A3)/B3*(exp(B3*(k4-L2))-exp(B3*(k3-L2))) - exp(alpha_s3+A3)/B3*(exp(B3*(k3-L2))-exp(B3*(k2-L2))) - exp(alpha_s2+A3)/B3*(exp(B3*(k2-L2))-exp(B3*(k1-L2))) - exp(alpha_s1+A1)*(exp(B1*L1)-1)/B1 - exp(alpha_s1+A2)*(exp(B2*(L2-L1))-1)/B2 - exp(alpha_s1+A3)*(exp(B3*(k1-L2))-1)/B3;
    end;
	...
	** average hazard ratio;
	...
	del5 = max( L2, min( (beta*L2*(gamma_t3_trt-gamma_t2_trt)+beta*L1*(gamma_t2_trt-gamma_t1_trt)-(beta*gamma_trt + alpha_trt))/(beta*gamma_t3_trt), t0) );
	del6 = min( t0, max( (beta*L2*(gamma_t3_trt-gamma_t2_trt)+beta*L1*(gamma_t2_trt-gamma_t1_trt)-(beta*gamma_trt + alpha_trt))/(beta*gamma_t3_trt), L2) );
    ...
    phi3 = sign(beta*gamma_t3_trt)*( (beta*gamma_t1_trt*L1+beta*gamma_t2_trt*(L2-L1)+(beta*gamma_trt + alpha_trt))**3 + (beta*gamma_t3_trt*(t0-L2)+beta*gamma_t2_trt*(L2-L1)+beta*gamma_t1_trt*L1+(beta*gamma_trt + alpha_trt))**3 - (beta*gamma_t3_trt*(del5-L2)+beta*gamma_t2_trt*(L2-L1)+beta*gamma_t1_trt*L1+(beta*gamma_trt + alpha_trt))**3 - (beta*gamma_t3_trt*(del6-L2)+beta*gamma_t2_trt*(L2-L1)+beta*gamma_t1_trt*L1+(beta*gamma_trt + alpha_trt))**3 )/(3*beta*gamma_t3_trt);
    c3 = sign(beta*gamma_t3_trt)*( beta*gamma_t3_trt*(t0-L2)**2/2 - beta*gamma_t3_trt*(del5-L2)**2/2 - beta*gamma_t3_trt*(del6-L2)**2/2 + (beta*gamma_t2_trt*(L2-L1)+beta*gamma_t1_trt*L1+(beta*gamma_trt + alpha_trt))*(L2+t0-del5-del6) );
 
extra = t0*(beta*gamma_trt + alpha_trt) + beta*( gamma_t1_trt*L1**2/2+gamma_t2_trt*(L2-L1)**2/2+gamma_t3_trt*(t0-L2)**2/2+gamma_t2_trt*(L2-L1)*(t0-L2)+gamma_t1_trt*L1*(t0-L1) );
  eta = 1e-3;

 estimate "Overall Effect eta" (phi1+phi2+phi3+extra*eta)/(c1+c2+c3+eta*t0);
